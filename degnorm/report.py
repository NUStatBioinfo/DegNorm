...
def render_report(data_dir, degnorm_data, bam_files,
    ...
    :param degnorm_data: dictionary with DegNorm parameters, data to report.
    :param bam_files: list of str names of input .bam files
    ...
    # ---------------------------------------------------------------------------- #
    files_df = DataFrame([bam_files, sample_ids]).T

    if (len(bam_files) == 1) and (os.path.isdir(bam_files[0])):
    ...
    # ---------------------------------------------------------------------------- #
    n_genes = len(degnorm_data.get('genes'))
    run_dat = {'NMF-OA SVD iterations': [degnorm_data.get('nmf_iter')]
               , 'DegNorm iterations': [degnorm_data.get('degnorm_iter')]
               , 'Downsample rate': ['1/{0}'.format(str(degnorm_data.get('downsample_rate')))]
               , 'Number of input genes': [n_genes]}
    ...
    # ---------------------------------------------------------------------------- #
    plot_dists = False
    if degnorm_data.get('rho').shape[0] > 1 and np.linalg.matrix_rank(degnorm_data.get('rho')) > 1:
        plot_dists = True
    ...
    # ---------------------------------------------------------------------------- #
    avg_dis = degnorm_data.get('rho').mean(axis=1)
    n_genes = min(top_n_genes, n_genes)
    hi_di_idx = avg_dis.argsort()[::-1][0:n_genes]
    lo_di_idx = avg_dis.argsort()[0:n_genes]

    hi_di_genes = [degnorm_data.get('genes')[hi_di_idx[i]] for i in range(n_genes)]
    lo_di_genes = [degnorm_data.get('genes')[lo_di_idx[i]] for i in range(n_genes)]
